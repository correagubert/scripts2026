    ...
    return (
      <section className='p-6 bg-gray-50 rounded-lg shadow-md'>
        ...
        <div className='mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3'>
            ...
            <input type='text' id='search' value={searchTerm} onChange={handleSearchChange} placeholder='Digite o nome ou identificador...' className='w-full sm:w1/2 p-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-cyan-600'/>
        </div>
        ...
        <ul className='space-y-4'>
            {
                filteredPatients.length > 0 ? (
                    filteredPatients.map((patient) => (
                        <li key={patient.id} className='p-4 bg-white rounded-lg shadow-sm border hover:shadow-md transition-shadow'>
                            ...
                            <Link to={`/paciente/${patient.id}`} className='inline-block mt-2 text-cyan-700 font-semibold hover:underline'>Ver detalhes</Link>
                        </li>
                    ))
                ):(
