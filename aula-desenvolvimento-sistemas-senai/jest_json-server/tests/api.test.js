...
// -----------------------------
// Exercício 11: Remoção de usuários criados por testes antigos
// -----------------------------
test("DELETE /users/ que exclua usuários além de uma length máxima de cinco usuários", async () => {
    const res = await fetch(`${baseUrl}/users/`);
    const data = await res.json();
...
// Teste 1 (clients)
test("GET /clients retorna 200", async () => {
    const res = await fetch(`${baseUrl}/clients`);
    expect(res.status).toBe(200);
});

// Teste 2 (clients)
test("GET /clients retorna um array", async () => {
    const res = await fetch(`${baseUrl}/clients`);
    const data = await res.json();
    expect(Array.isArray(data)).toBe(true);
});